.data
arr:    .word     5 : 441       # storage for 10x10 matrix of words
dot:   .asciiz	  ". "
dash:  .asciiz	  "- "
bar:   .asciiz	  "| "
one:   .asciiz	  "1 "
...
Jloop:	
	...
	li $t6, 4
	beq $t8, $t6, SkipSpace #skip if arr[i,j] == 4 	
	li $t6, 5
	beq $t8, $t6, SkipSpace #skip if arr[i,j] == 5	
	...
	#draw 1 inside box
	bne $t3, $zero, SkipOne #skip if i  != even
	...
	#draw 2 inside box
	bne $t3, $zero, SkipTwo #skip if i != even
	...
	bne $t8, $zero, SkipSpaceAgain #skip if arr[i, j] != 0 
	add $t6, $t3, $t4
	li $t7, 2
	beq $t6, $t7, SkipSpaceAgain
	la $a0, space
	addi $v0, $zero, 4
	syscall
	SkipSpaceAgain:
